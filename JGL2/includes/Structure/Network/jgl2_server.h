...
namespace jgl
{
	namespace Network
	{
		...
		{
		private:
			using ServerConnection = jgl::Network::Connection<TServerMessageEnum>;
			using ServerMessage = jgl::Network::Message<TServerMessageEnum>;
		...
		protected:
			...
			void _onMessageReception(ServerConnection* p_client, ServerMessage& p_msg)
			{
			...
			jgl::Bool _validateClientConnection(ServerConnection* p_client, ServerMessage& p_msg)
			{
				jgl::Long key;
		...
		public:
			...
			ServerConnection* connection(jgl::Long p_id)
			{
			...
			void sendTo(ServerConnection* p_client, const ServerMessage& p_msg)
			{
			...
			void sendToAll(const ServerMessage& p_msg, ServerConnection* p_ignore = nullptr)
			{
			...
			void sendToArray(const ServerMessage& p_msg, const std::vector<ServerConnection*>& p_list)
			{
		...
		template <typename TServerMessageEnum>
		class ServerSingleton : public jgl::Singleton< jgl::Network::Server<TServerMessageEnum> >
		{
		private:
			using ServerType = jgl::Network::Server<TServerMessageEnum>;
		public:
			static ServerType* instanciate() = delete;
			static ServerType* instanciate(jgl::ULong p_port)
			{
				if (jgl::Singleton<ServerType>::_instance == nullptr)
				{
					jgl::Singleton<ServerType>::_instance = new ServerType(p_port);
				}
				return (jgl::Singleton<ServerType>::_instance);
			}
		};
	}
}